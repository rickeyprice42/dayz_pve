...
class CustomLoadingScreenData
{
	...
	int LoadingBarColor = ARGB(255, 126, 138, 112);
	bool UseCustomHints = true;
	string HintIconPath = "Grimhold/GUI/icons/circle_info.edds";
	...
	void CustomLoadingScreenData()
	{
		...
		JsonFileLoader<array<ref CustomLoadingScreenBackground>>.JsonLoadFile("Grimhold/Scripts/Data/LoadingImages.json", m_ExBackgrounds);
	}
};

modded class UiHintPanel
{
	protected const string m_ExDataPath = "Grimhold/Scripts/Data/Hints.json";
...
modded class LoadingScreen
{
	...
	protected void HideVanillaLoadingLogos()
	{
		if (m_ImageLogoMid)
		{
			m_ImageLogoMid.Show(false);
			m_ImageLogoMid.Unlink();
		}
	...
	void LoadingScreen(DayZGame game)
	{
		...
		m_ExHintIcon = ImageWidget.Cast(game.GetLoadingWorkspace().CreateWidgets("Grimhold/GUI/layouts/hint_icon.layout", m_ExPanelWidget));
		...
		if (m_ExCustomLoadingScreenData.ShowLogo && m_ExCustomLoadingScreenData.LogoPath != string.Empty)
		{
			...
			m_ImageLogoCorner = ImageWidget.Cast(game.GetLoadingWorkspace().CreateWidgets("Grimhold/GUI/layouts/logo.layout", m_ExPanelWidget));
		...
		if (m_ExCustomLoadingScreenData.LoadingBarColor != 0)
		{
			...
			m_ProgressLoading = ProgressBarWidget.Cast(game.GetLoadingWorkspace().CreateWidgets("Grimhold/GUI/layouts/loading_bar.layout", m_ExPanelWidget));
